...
PRECISION = 10 ** 18
FEE = 0.1435  # %  https://arbitrum.curve.fi/tricrypto/, Sep 26 2021
# =========================
...
# PRECISION = 10 ** 18
# FEE = 0.124  # %  https://polygon.curve.fi/atricrypto/deposit, Sep 26 2021
# # =========================
...
for i in tqdm(range(len(blockids))):
    ...
    for txhash in blockdata:
        ...
        lp_revenue_usd = volume_usd * FEE / 100

        these_vols += volume_usd

        volumes_by_date[time_exchanged] = volume_usd
        fees_by_date[time_exchanged] = lp_revenue_usd
...
dffee = pd.DataFrame.from_dict(fees_by_date, orient="index").sort_index()

df[f"{TYPE}_LP_REV"] = dffee[0]
